...
public class StatsDisplayerRoadGenerator : MonoBehaviour
{
    ...
    // Parameters
    public float offTrackXTEThreshold; 
    ...
    private void InitializeStats(bool createFile)
    {
        ...
        if (writeLog)
        {
            ...
            System.IO.File.AppendAllLines(frameLogPath, new string[] { "frameId,xte,steering,throttle,speed,acceleration,x,y,isOffTrack" });
        }
        ...
        if (statsPanel != null)
        {
            ...
            if (labels.Length >= 8)
            {
                currWaypointLabel = labels[0];
                timeLabel = labels[1];
                outOfTrackLabel = labels[2];
                currSpeedLabel = labels[3];
                avgSpeedLabel = labels[4];
                xteLabel = labels[5];
                avgXteLabel = labels[6];
                xteMaxLabel = labels[7];
            }
            else {
                Debug.Log("More labels than variables");
            }
        }
        else {
            Debug.Log("StatsPanel is null");
        }
    }
    ...
    {
        string frameStats = frameId + "," + xte + "," + car.GetSteering() + "," + car.GetThrottle() + "," + car.GetVelocity().magnitude + "," + car.GetAccel().magnitude + "," + car.transform.position.x + "," + car.transform.position.z  + "," + isOffTrack;
